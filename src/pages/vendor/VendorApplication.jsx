...
export default function VendorApplication() {
  const vendorApplicationSchema = Joi.object({
    ...
    shopMobile: Joi.string().length(10).trim().required().messages({
      "number.pattern.base": "Do not contain letters",
      "string.empty": "shopMobile is required",
    }),

    openingTimes: Joi.string().trim().required(),

    closingTimes: Joi.string().trim().required(),
    ...
    idNumber: Joi.string()
      ...
      .required()
      .messages({
        "string.pattern.base": "Do not contain letters",
        "string.empty": "empty is required",
      }),

    juristicId: Joi.string()
      .pattern(/^[0-9]+$/)
      .length(13)
      .trim()
      .required()
  ...
  const validatevendorApplication = (input) => {
    const { error } = vendorApplicationSchema.validate(input, {
      abortEarly: false,
    });
    console.log(error);
    if (error) {
  ...
  const inputTag = [
    ...
    { id: 4, name: "shopMobile", label: "restaurant tel.", picture: true },
  ...
  const hdl_submit = async (e) => {
    e.preventDefault();
    console.log("error ma law ton button");
    const validationErrors = validatevendorApplication(appInput);
    if (validationErrors) {
  ...
  return (
    <section className="section my-24 flex flex-col gap-12">
      <div className="container flex justify-center gap-8">
        <div
          className={`flex flex-col gap-2 cursor-pointer text-sm rounded-lg px-8 py-2 justify-center items-center border-solid border-2 border-gray-300 text-gray-400 desktop:text-lg hover:bg-primary-50 ${
        ...
        >
          <PersonalIcon
            className={`fill-current ${
              selectedItem === "personal" ? "text-primary-500" : ""
            }`}
          />
          <p>Person</p>
        </div>
        ...
        <div
          className={`flex flex-col gap-2 cursor-pointer text-sm rounded-lg px-8 scroll-py-24 justify-center items-center border-solid border-2 border-gray-300 text-gray-400 desktop:text-lg hover:bg-primary-50 ${
            selectedItem === "group"
              ? "border-primary-500 text-primary-500"
              : ""
          }`}
        ...
        >
          <GroupIcon
            className={`fill-current ${
              selectedItem === "group" ? "text-primary-500" : ""
            }`}
          />
          <p>Juristic</p>
        </div>
      ...
      <form
        key="1"
        className="container min-w-[240px] flex flex-col gap-4"
        onSubmit={hdl_submit}
      >
        {inputTag.map((el) => {
          ...
          return (
            <div key={el.id} className=" flex flex-col ">
              ...
              {el.name && <InputErrorMessage message={error[el.name]} />}
              ...
              {el.name === "juristicId" && (
                <div className="flex flex-col">
                  ...
                  <input
                    ...
                    error={error.juristicId}
                  />
                  {error.juristicId && (
                    <InputErrorMessage message={error.juristicId} />
                  )}
                </div>
              )}
              {el.name === "idNumber" && (
                <div className="flex flex-col">
                  {idCardFile ? (
                    <img src={URL.createObjectURL(idCardFile)} />
                  ) : null}
