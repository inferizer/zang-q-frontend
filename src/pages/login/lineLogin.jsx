...
export default function LinePage() {
  ...
  const handleLogin = () =>
    liff
      .init({ liffId: "2001390459-y9GRwgp7" }, () => {
        if (liff.isLoggedIn()) {
          liff.ready.then(() => {
            liff.getProfile().then((res) => {
              axios.post("/auth/loginLine", res).then((res) => {
                console.log(res.data.user);
                const token = encodeURIComponent(res.data.accessToken);
                window.location
                  .replace(`http://localhost:5173/line-callback?token=${token}
  ...
  const handlelogout = () =>
    liff.init({ liffId: "2001390459-y9GRwgp7" }).then(() => {
      liff.logout();
    });
